...
%DEMO07_ZAK_PHASE_STPWE Spatial-BZ Zak phase of continuum ST-PWE bands.
%
% The static calculation is a well-isolated reference. The driven
% calculation illustrates the same workflow and reports its minimum Wilson
% link, which must be checked before interpreting a single-band invariant.
%
% The Zak phase depends on the unit-cell origin. The Fig. 2 cell starts at
% a layer boundary rather than an inversion center, so it need not appear
% as exactly 0 or pi in this coordinate convention.
%
% IMPORTANT: this loop integrates over spatial Bloch momentum k. It is not
% the temporal Zak phase of a spatially uniform photonic time crystal,
% whose Bloch coordinate is quasifrequency and whose eigenvalue is k.
...
title(tl,'Spatial-BZ biorthogonal Wilson loop for ST-PWE');
outputFile = fullfile(rootDir,'output','demo07_zak_phase_stpwe.png');
try
