...
%DEMO01_REPRODUCE_FIG2_STPWE Reproduce Park-Min Fig. 2 with ST-PWE.
%
%   demo01_reproduce_fig2_stpwe('quick')
%   demo01_reproduce_fig2_stpwe('paper')
%
% The color is the m=0 Floquet-sector participation. It is a transparent,
% reproducible proxy for the paper's projection-based static-band weight.
...
fprintf('Demo 01: %d k points, generalized matrix size %d x %d.\n', ...
    Nk, 2*sys.S, 2*sys.S);
for ik = 1:Nk
...
epsCoeff0 = @(n) stm_fig2_eps_coeff(0,n,p);
...
epsPlot = stm_fig2_epsilon(XX, Tau*p.T, p);
...
title(tl, 'Spatiotemporal PWE reproduction of Park-Min Fig. 2');
outputFile = fullfile(rootDir, 'output', ...
    ['demo01_fig2_stpwe_' lower(quality) '.png']);
save_demo_figure(fig, outputFile);
fprintf('Saved %s\n', outputFile);
end
