...
numerics.zeroKPropagationDirection = read_override_finite( ...
    overrides,'zeroKPropagationDirection',1);
if ~ismember(numerics.zeroKPropagationDirection,[-1 1])
    error('overrides.zeroKPropagationDirection 必须等于 -1 或 +1。');
end
...
scanCfg.zeroKPropagationDirection = numerics.zeroKPropagationDirection;
...
result.columnNormalizedPower = fftBands.columnNormalizedPower;
result.globalNormalizedPower = fftBands.globalNormalizedPower;
...
result.sourceInitialization = struct( ...
    'propagationDirections',scanResult.propagationDirections, ...
    'zeroKPropagationDirection',scanResult.zeroKPropagationDirection, ...
    'carrierOmegaYee',scanResult.carrierOmegaYee, ...
    'carrierGroupSpeedYee',scanResult.carrierGroupSpeedYee);
...
result.samplingMetadata = struct('nFft',fftBands.nFft, ...
    ...
    'analysisTimeRangeFs',fftBands.analysisTimeRange, ...
    'analysisRows',fftBands.analysisRows, ...
    'probeCount',fftBands.probeCount, ...
    'windowName',fftBands.windowName, ...
    'windowEnbwBins',fftBands.windowEnbwBins, ...
    'window',fftBands.window, ...
    'spectralNormalization',fftBands.spectralNormalization, ...
