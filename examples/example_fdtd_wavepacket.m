...
%EXAMPLE_FDTD_WAVEPACKET FDTD simulation of a wave packet in the Park-Min crystal.
%
% A Gaussian wave packet propagates through the spacetime-periodic medium
% described by Park and Min (Fig. 2). The modulation is present throughout
% the simulation domain. The probe spectrogram reveals Floquet sidebands
% generated by the spatiotemporal modulation.
%
% See also FDTD1D_DB, STM_EPSILON_PARK_MIN, STM_STFT
...
cfg.dt = dt;
cfg.nSteps = nSteps;
cfg.epsFun = @(xq,tq) stm_permittivity_modulated_slab(xq,tq,p);
...
title(tl,'D/B Yee-FDTD for the Park-Min spacetime crystal');
outputFile = fullfile(rootDir,'output', ...
    'example_fdtd_wavepacket.png');
try
